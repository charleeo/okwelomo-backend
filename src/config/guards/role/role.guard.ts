...
export class RoleGuard implements CanActivate {
  ...
  canActivate(
    ...
    const actions = this.reflector.get<ActionEnums>('actions', context.getHandler())
  ...
  async checkRole(actions:ActionEnums,token:string):Promise<boolean>{
    ...
    if(actions && actions.length > 0 ) {
      ...
      const roles:string[] = verifyToken(token.replace("Bearer","").trim()).payload.actions
      let isFounded = roles.some( role => actionsArray.includes(role) )
      if(!isFounded ){
        status=false
      }
    }else{//if this guard is in actoin there should be some metadata attached else, there will be an error
      status=false
    }
